    ...
    @if (session('message'))
        <div class="flex flex-col bg-green-600 p-1 mb-2 text-white text-sm rounded-sm">        
            <div class="flex row justify-between items-center">
                ...
                <a href="/tags/" class="cursor-pointer" title="Close">
                ...
                <div class="overflow-x-auto">
                    ...
                    <table class="min-w-full ">
                        ...
                        <tbody>
                            ...
                            @foreach ($tags as $tag)
                                <tr
                                    class="text-black text-sm leading-6 {{in_array($tag->id, $okselections) ? 'bg-green-200' : 'even:bg-zinc-100 odd:bg-zinc-50'}} transition-all duration-1000 hover:bg-yellow-200">                                        
                                    <td class="p-2 text-center"><input wire:model.live="selections" type="checkbox"
                                            ...
                                            value={{ $tag->id }}
                                            id={{ $tag->id }}
                                            {{ in_array($tag->id, $selections) ? 'checked' : '' }}
                                            >
                                        </td>
                                    <td class="p-2 {{ $column == 'id' ? 'bg-yellow-300 font-bold text-black transition-all duration-1000' : '' }}">{{ $tag->id }}</td>
                                    ...
                                    <td class="p-2 {{ $column == 'created_at' ? 'bg-yellow-300 font-bold text-black transition-all duration-1000' : '' }}">{{ date('d-m-Y', strtotime($tag->created_at)) }}</td>
                                    <td class="p-2 {{ $column == 'updated_at' ? 'bg-yellow-300 font-bold text-black transition-all duration-1000' : '' }}">{{ date('d-m-Y', strtotime($tag->updated_at)) }}</td>
                                    
                                    <td class="p-2">
                                        <div class="flex justify-center items-center gap-2">
                                            <!-- Show -->
                                            <a href="{{ route('tags.show', $tag) }}" title="Show">
                                                <i
                                                    class="fa-solid fa-circle-info text-amber-600 hover:text-black transition duration-1000 ease-in-out"></i>
                                            </a>
                                            <!-- Edit -->
                                            <a href="{{ route('tags.edit', $tag) }}" title="Edit">
